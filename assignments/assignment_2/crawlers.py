...
def clean_branch_folder(provider, branch):
    folder = os.path.join("data", provider, branch)
    if os.path.exists(folder):
        for f in os.listdir(folder):
            os.remove(os.path.join(folder, f))


def download_and_save_reports(reports, provider, type, clean=False, url = "https://url.publishedprices.co.il"):
    ...
    for branch, report in reports.items():
        ...
        save_dir = os.path.join(f"data/{provider}", branch)
...
def crawl():
    ...
    for i in range(3):
        ...
        driver.find_element(By.ID, "username").send_keys(usernames[i])
        ...
        download_and_save_reports(latest_prices, providers[i], "Price", True)
        download_and_save_reports(latest_promos, providers[i], "Promo", False)

        print(f"Scrawling finished successfully!")
        time.sleep(5)
