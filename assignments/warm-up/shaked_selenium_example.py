...
def crawl():
    ...
    article["updated_up"] = author_details[2]
    article["content"] = soup.find_all(class_="article-content")[0].get_text(strip=True)
    article["images"] = soup.find(class_="post__image")["src"]
    
    # video = soup.select('.video-playlist__container')
    # video = soup.find(class_='.video-playlist__container')
    # print(video)
