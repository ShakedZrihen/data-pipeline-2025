    ...
    if not isinstance(items, list) or len(items) < 1:
        ...
        for i, it in enumerate(items):
            if not isinstance(it, dict):
                ...
                continue
            if not isinstance(it.get("product"), str) or len((it.get("product") or "").strip()) < 1:
                msgs.append(f"item[{i}].product is too short")
            if not isinstance(it.get("unit"), str) or len((it.get("unit") or "").strip()) < 1:
...
def doc_to_rows(doc: Dict[str, Any]):
    ...
    for it in doc["items"]:
        ...
        bc = it.get("barcode")
        if bc:
            row["barcode"] = str(bc).strip()

        if doc["type"] == "promoFull":
            row["promo_price"] = float(it.get("price", 0) or 0)
            row["promo_text"] = it.get("promo_text") or it.get("product")
