...
class SimpleSQSConsumer:
    ...
    def _prepare_item_for_db(self, item: Dict[str, Any], message_id: str, total_items: int) -> Dict[str, Any]:
        ...
        unit_price = self._safe_float(item.get('UnitOfMeasurePrice', item.get('unit_price', 0)))
        min_quantity = self._safe_float(item.get('MinQuantity', item.get('min_quantity', 0)))
        ...
        return {
            ...
            'product_code': item.get('ItemCode', item.get('item_code', '')),
