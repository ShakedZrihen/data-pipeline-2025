...
from db_tracker import put_last_run
from log_utils import log, log_exception
from s3_writer import write_json_to_s3

s3 = boto3.client("s3", endpoint_url=ENDPOINT_URL, region_name=AWS_REGION)

KEY_RE = re.compile(r"^providers/(?P<provider>[^/]+)/(?P<branch>[^/]+)/(?P<filetype>pricesFull|promoFull)_(?P<timestamp>\d{14})\.gz$")
...
def _write_error(provider, branch, file_type, ts_dt, key, err):
    # Write a small error artifact alongside normal outputs to diagnose quickly
    payload = {
    ...
    try:
        ...
        log_exception("direct_json_write_failed_error_payload", e)

def _process_object(key: str):
    parsed = _parse_key(key)
    if not parsed:
        log("skip_non_matching_key", key=key); return
    provider, branch, file_type, ts_dt = parsed
    if file_type not in SUPPORTED_TYPES:
        log("skip_unsupported_type", key=key, file_type=file_type); return
    ...
    try:
        ...
        # always write JSON so we can see even empty results
        try:
            key_json = write_json_to_s3(payload)
            log("direct_json_write_ok", key=key_json)
        except Exception as e:
            log_exception("direct_json_write_failed", e)

        # track last run (best effort)
        try:
            put_last_run(provider, branch, file_type, payload["timestamp"])
        except Exception as e:
            log_exception("ddb_put_last_run_failed", e)

    except Exception as e:
        log_exception("process_failed", e, key=key)
        # write a tiny error artifact so you can see failures in S3/Json/
        try:
            _write_error(provider, branch, file_type, ts_dt, key, e)
        except Exception as _:
            pass

def handler(event, context):
    log("handler_start", mode="s3-event" if event and "Records" in event else "unknown")
    for rec in (event.get("Records", []) if event else []):
        if rec.get("s3", {}).get("bucket", {}).get("name") != S3_BUCKET:
            continue
        key = rec["s3"]["object"]["key"]
        _process_object(key)
    log("handler_done")
