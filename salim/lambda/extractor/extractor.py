...
def main():
    ...
    for key in keys:
        ...
        previous_file = processed_map.get(id_key)

        if previous_file and previous_file.get("timestamp") and previous_file["timestamp"] >= current_timestamp:
            continue
        ...
        if previous_file and previous_file.get("json"):
            try:
                (OUT_ROOT / previous_file["json"]).unlink(missing_ok=True)
            except Exception as exception:
                print(f"[WARN] Could not remove old JSON {previous_file['json']}: {exception}")
